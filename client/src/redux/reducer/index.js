...
const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actions.ORDER_RECIPES_ASCENDING:
      return {
        ...state,
        recipes: state.recipes.slice().sort((r1, r2) => {
    ...
    case actions.FILTER_BY_DIET:
      return {
        ...
        //   const filterDiets = r.diets.filter((d) => d === action.payload);

        //   if (filterDiets.length === action.payload.length) return true;
        // }),
        // filterRecipes: state.recipes.filter((r) =>
        //   r.diets.includes((d) => {
        //     for (let i = 0; i < action.payload.length; i++) {
        //       if (d === action.payload[i]) return true;
        //     }
        //   })
        // ),

        filterRecipes: state.recipes.filter((r) =>
          action.payload.some((p) => r.diets.includes(p))
        ),
      };
    default:
      return state;
